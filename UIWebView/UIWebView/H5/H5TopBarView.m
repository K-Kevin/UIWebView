...
@interface H5TopBarView()

@property (nonatomic, strong) H5TopBarInfo *topBarInfo;
...
- (void)topBarTapAction:(id)sender {
    ...
    H5TopBarTapEvents tapTag = button.tag;
    if (_delegate && [_delegate conformsToProtocol:@protocol(H5TopBarViewDelegate)]) {
        if ([_delegate respondsToSelector:@selector(h5TopBarView:buttonTapActionTapType:)]) {
            [_delegate h5TopBarView:self buttonTapActionTapType:tapTag];
        }
    }
}
...
- (UIButton *)backButton {
    if (!_backButton) {
        ...
        _backButton.tag = H5TopBarTapEventsBack;
        [_backButton setImage:[UIImage imageNamed:@"Image"] forState:UIControlStateNormal];
...
- (UIButton *)closeButton {
    if (!_closeButton) {
        ...
        _closeButton.tag = H5TopBarTapEventsClose;
...
- (UIButton *)refreshButton {
    if (!_refreshButton) {
        ...
        _refreshButton.tag = H5TopBarTapEventsRefresh;
...
- (H5TopBarInfo *)topBarInfo {
    if (!_topBarInfo) {
        _topBarInfo = [[H5TopBarInfo alloc] init];
    }
    return _topBarInfo;
}
